...
/** #RRGGBB→rgba(r,g,b,a) に変換（#RGBにも対応） */
...
export default function NetworkHeatmap({
	...
	const generateNetworkData = () => {
		...
		const publications = [
			{
				id: "coral-climate",
				name: "Coral Climate Resilience",
				type: "publication" as const,
			},
			{
				id: "deep-carbon",
				name: "Deep Ocean Carbon",
				type: "publication" as const,
			},
			{
				id: "microplastics",
				name: "Microplastic Impact",
				type: "publication" as const,
			},
			{
				id: "acidification",
		...
		const processedNodes: Node[] = allNodes.map((node, index) => {
			...
			if (node.type === "researcher") {
				...
				radius = 25;
				color = cssVar("--IO_DARK_BLUE"); // Ocean blue
			} else if (node.type === "publication") {
				// Publications in inner circle
				const angle =
					((index - researchers.length) / publications.length) *
					2 *
					Math.PI;
				const distance = 60;
				x = centerX + Math.cos(angle) * distance;
				y = centerY + Math.sin(angle) * distance;
				radius = 18;
				color = cssVar("--INFOGRAPHIC_MID_LIGHT_BLUE"); // Cyan
			} else {
				...
				y = centerY + Math.sin(angle) * distance;
				radius = 30;
				color = cssVar("--UWA_BLUE"); // Dark cyan
			}
	...
	useEffect(() => {
		...
		nodes.forEach((node) => {
			...
			ctx.arc(
				node.x,
				node.y,
				isHovered ? node.radius * 1.2 : node.radius,
				0,
				2 * Math.PI
			);
			ctx.fill();
